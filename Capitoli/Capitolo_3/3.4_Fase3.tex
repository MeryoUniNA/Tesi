...
\begin{description}
    ...
    Questa direttiva affronta uno dei problemi di fallback più
    diffusi nella generazione di codice tramite LLM. Storicamente, se
    lasciati liberi, i modelli tendono a generare frammenti di
    codice o diff testuali sparsi all'interno di discorsi in
    prosa libera. Questo comportamento è incompatibile con un'applicazione automatizzata.
    La formulazione perentoria, rafforzata da una negazione esplicita (\texttt{"Never
    output raw diffs"}), forza l'agente a delegare l'atto della
    manipolazione del codice unicamente allo strumento strutturato \texttt{apply\_ast\_patch}.
    Senza questo vincolo, il modello potrebbe teoricamente enunciare una
    soluzione corretta ma fallire nell'applicarla sul file fisico, vanificando
    la fase di \texttt{COMPILE} successiva poiché il sorgente rimarrebbe
    di fatto inalterato.

    \item[Il Vincolo di Minimalità:]
    (\texttt{"Keep fixes MINIMAL..."}):
    Questa regola recepisce un principio cardine della teoria dell'Automated
    Program Repair (APR). Un assunto fondamentale della disciplina stabilisce
    che le patch minime hanno una probabilità nettamente inferiore
    di indurre regressioni su comportamenti del sistema che non
    sono coperti dalla suite di test utilizzata per validare
    il bug. Ricerche empiriche su sistemi allo stato dell'arte
    (come AlphaRepair) hanno dimostrato statisticamente che le correzioni di
    dimensione unitaria (le cosiddette patch \textit{single-hunk}, ovvero concentrate su
    un singolo blocco logico contiguo) risultano possedere un grado
    di plausibilità e generalizzazione di gran lunga superiore rispetto
    alle modifiche distribuite (\textit{multi-hunk}). Forzando il modello a privilegiare
    la minima perturbazione necessaria a ripristinare il comportamento atteso,
    si mitiga drasticamente il problema dell'overfitting sui test falliti.
\end{description}
...
\begin{description}
    ...
    formidabili. La firma esatta del metodo da invocare potrebbe non
...
Un aspetto ingegneristico affascinante della fase \texttt{EXPLORE} riguarda il suo
meccanismo di terminazione. La durata di questa fase --- misurabile
in numero di round di invocazione degli strumenti --- non
è rigidamente temporizzata, ma è deterministica solo in senso probabilistico.
...
\begin{description}
    \item[Completezza sintattica garantita:] I sistemi basati su diff soffrono intrinsecamente
    del problema dell'ambiguità contestuale. In un ciclo multi-tentativo come quello
    di \texttt{PatchAgent}, il codice sorgente muta a ogni iterazione fallita.
    Se l'agente generasse un diff, le sue righe di contesto
    (le righe non modificate usate come ancore per capire dove
    applicare la patch) potrebbero non corrispondere più esattamente al file
    bersaglio modificato, causando il fallimento tecnico dell'applicazione della patch. Al
    contrario, un metodo riscritto nella sua interezza è un blocco
    logico autonomo, la cui applicazione non soffre di alcuna ambiguità
    posizionale ed è sempre riapplicabile.

    \item[Compatibilità nativa con Tree-sitter:] L'infrastruttura di manipolazione del codice (\texttt{ast\_applier.py})
    ...
    nodo \texttt{method\_declaration} o \texttt{constructor\_declaration}) e isola il suo \textit{text span},
    ...
    ``pigre'' o iper-focalizzate. Se istruiti a generare solo la modifica,
    ...
    il rischio di errori di scope o sintassi sbilanciata.
\end{description}
...
Il giudizio di un \textit{Large Language Model} (LLM) in
merito alla correttezza del codice prodotto è intrinsecamente probabilistico:
...
risulta sintatticamente e semanticamente valida sotto il profilo della
...
\begin{lstlisting}[language=Python, caption={Logica di interfaccia dello strumento \texttt{compile\_check}.}, label={lst:tool_compile_check}]
...
Un aspetto ingegneristico cruciale visibile in questa funzione è
...
\begin{lstlisting}[language=Python, caption={Logica di interfaccia dello strumento \texttt{run\_tests}.}, label={lst:tool_run_tests}]
...
esecuzione casuale, ma si fonda su tre pilastri ingegneristici
di adattamento: una modulazione adattiva della temperatura del modello,
un isolamento rigoroso del workspace e l'iniezione della memoria
...
\begin{lstlisting}[language=Python, caption={Definizione dello schedule di temperatura per i retry in \texttt{config.py}.}, label={lst:temp_schedule}]
# shared/config.py
LLM_TEMPERATURE_SCHEDULE = [0.1, 0.2, 0.4]
COMPILE_ERROR_TEMP_DOWNSHIFT = 0.1
\end{lstlisting}

La scelta ingegneristica di questi valori specifici (\texttt{0.1}, \texttt{0.2} e
...
\begin{description}
    ...
    formale che preserva la correttezza della sintassi e l'uso
    valido delle API Java.
\end{description}
...
\begin{lstlisting}[language=Python, caption={Funzione di isolamento del workspace sorgente prima di un nuovo tentativo.}, label={lst:create_patched_copy}]
...
vitale per la robustezza ingegneristica dell'agente: essa garantisce che
...
\subsubsection{La Memoria Conversazionale: \texttt{conversation\_store.py} e lo Schema ChatRepair}
\label{subsubsec:3.4.2.7}
L'evoluzione dell'\textit{Automated Program Repair} basato su Large Language Models
ha evidenziato un limite architetturale catastrofico nei sistemi multi-tentativo
...
comparsa del cosiddetto \textit{loop sterile} (formalizzato nello sviluppo di
\texttt{HybridRepair} come problema P3): il modello, deterministico per natura
...
il paradigma \textit{ChatRepair}. Questo schema prevede che la storia
...
\begin{lstlisting}[language=Python, caption={Salvataggio della memoria conversazionale in formato \texttt{JSONL}.}, label={lst:conversation_save}]
...
La scelta ingegneristica del formato JSONL in sostituzione a
...
\begin{lstlisting}[language=Python, caption={Estrazione del riassunto per il \textit{negative prompting} tra tentativi.}, label={lst:get_failure_summary}]
...
La stringa risultante da questo costrutto è un capolavoro
di ingegneria dei prompt. Invece di chiedere genericamente al
modello di ``cercare un'altra via'', il riassunto esplicita esattamente
quale azione ha compiuto l'assistente nell'ultimo messaggio utile del
tentativo fallito e quale errore granulare ha scatenato (sia
esso lo stack trace di JUnit o l'errore verboso
di compilazione).
...
loop sterile che affliggeva la prima generazione del software
(v1) e rendendo le iterazioni di \texttt{HybridRepair} (v2) effettivamente
progressive.
...
\begin{lstlisting}[language=Python, caption={Gestione del loop conversazionale e chiamate agli strumenti in \texttt{chat\_with\_tools()}.}, label={lst:chat_with_tools}]
...
\begin{description}
    ...
    loop. Questa scelta di design ingegneristico permette al chiamante
    di livello superiore (il metodo \texttt{PatchAgent.\_run\_single\_attempt()}) di avere sempre
...
quale agisce da rigoroso meccanismo di sicurezza (\textit{safety limit}).
...
\begin{lstlisting}[language=Python, caption={Meccanismo di backoff esponenziale per la gestione del rate-limiting.}, label={lst:call_with_retry}]
...
il protocollo ingegneristico ufficialmente raccomandato per gestire l'eccezione \texttt{RateLimitError}.
...
problem} (problema della mandria tonante). Se tutte le richieste
...
dello stato attraverso il pattern \textit{Context Object} (definito dalla
Gang of Four). Invece di passare singolarmente i percorsi delle
...
\begin{lstlisting}[language=Python, caption={Definizione del \texttt{ToolContext} tramite dataclass.}, label={lst:tool_context}]
@dataclass
class ToolContext:
    bug_dir: Path
    patched_dir: Path
...
L'iniezione di questo \texttt{ToolContext} riduce drasticamente il \textit{coupling} (accoppiamento)
...
\begin{lstlisting}[language=Python, caption={Strumento per la lettura formattata del codice sorgente.}, label={lst:tool_read_file}]
...
\begin{lstlisting}[language=Python, caption={Strumento di ricerca globale nel progetto target.}, label={lst:tool_search_symbol}]
...
ingegneristicamente fondamentale poiché ignora i file binari (come i file
...
trovare tutte le assegnazioni di una variabile risultata \texttt{null} (cruciale
per i bug di tipo \texttt{UNINITIALIZED\_FIELD}); o individuare eventuali implementazioni
...
\begin{lstlisting}[language=Python, caption={Estrazione dinamica delle firme dei metodi pubblici.}, label={lst:tool_get_method_signature}]
...
\begin{lstlisting}[language=Python, caption={Applicazione strutturale della patch sul \textit{Concrete Syntax Tree}.}, label={lst:tool_apply_ast_patch}]
...
\label{subsec:3.4.5}
L'approccio alla modifica del codice sorgente rappresenta uno
degli snodi architetturali più delicati nella progettazione di
un sistema di Automated Program Repair. Tradurre un'intenzione
correttiva in una mutazione fisica del file richiede
un meccanismo che garantisca non solo l'accuratezza della
modifica, ma anche la validità formale dell'artefatto risultante.
Per soddisfare i severi requisiti di robustezza di
\texttt{HybridRepair}, l'architettura adotta la manipolazione diretta del \textit{Concrete
Syntax Tree} (CST) avvalendosi della libreria Tree-sitter, distaccandosi
nettamente dai paradigmi testuali classici.

\subsubsection{I Limiti Fondamentali degli Approcci Testuali}
\label{subsubsec:3.4.5.1}
Prima di esplorare le specificità tecniche di Tree-sitter,
è imperativo analizzare criticamente le ragioni per cui
gli approcci tradizionali --- manipolazione tramite espressioni regolari
(regex), diff unificati e operatori di mutazione ---
risultano intrinsecamente inadeguati per un sistema APR di
alta affidabilità.

\paragraph{Il fallimento delle espressioni regolari e il problema del bilanciamento}
Le espressioni regolari operano analizzando flussi monodimensionali di
caratteri. Questo approccio si scontra frontalmente con la
natura intrinsecamente gerarchica e ricorsiva del codice sorgente
Java. Un costrutto come un metodo non è
una semplice sequenza lineare, ma una struttura annidata
che può includere blocchi condizionali \texttt{if/else}, cicli \texttt{for/while},
blocchi \texttt{try/catch/finally}, oltre a costrutti complessi come classi
anonime o espressioni lambda. Ciascuno di questi elementi
introduce nuovi livelli di parentesi graffe che devono
essere rigorosamente bilanciati.

Dal punto di vista della teoria dei linguaggi
formali, il problema del bilanciamento delle parentesi appartiene
alla classe dei linguaggi liberi dal contesto (\textit{context-free}),
mentre le espressioni regolari possono descrivere unicamente linguaggi
regolari. Una regex semplicistica (come \verb|\w+\s*\([^)]*\)\s*\{[^}]*\}|) è in
grado di catturare solo metodi piatti, fallendo inesorabilmente
di fronte al primo blocco condizionale annidato. Storicamente,
la prima iterazione architetturale (v1) di \texttt{HybridRepair} tentava
di arginare questa limitazione combinando l'uso di regex
con contatori manuali di parentesi graffe (\texttt{\_count\_braces()}). Tuttavia,
questo approccio euristico scatenava il cosiddetto ``problema P1'':
la generazione di patch contenenti parentesi sbilanciate o
dichiarazioni di metodi frammentate o duplicate, costringendo il
sistema a scartare un elevato numero di tentativi
validi.

\paragraph{Il fallimento dei diff unificati nei sistemi multi-tentativo}
Il formato \textit{unidiff} (standardizzato in strumenti come \texttt{git diff}
e adottato da vecchi sistemi APR) codifica una
modifica definendo un blocco testuale (l'\textit{hunk}) ancorato a
specifiche righe di contesto preesistenti. Questo modello si
fonda su un'assunzione di immutabilità del contesto: le
righe circostanti la modifica devono coincidere in modo
esatto con quelle del file bersaglio.

In un agente iterativo guidato da un LLM,
questa assunzione crolla miseramente. L'agente esplora molteplici variazioni,
e i diff generati dal modello spesso fanno
riferimento al codice sorgente fallato originale (fornito nel
prompt iniziale). Tuttavia, al procedere dei round esplorativi,
il file bersaglio può accumulare piccole variazioni o
trovarsi in uno stato transitorio alterato. Il disallineamento
posizionale che ne consegue causa il rigetto del
diff in fase di applicazione, scaturendo in un
falso negativo: la logica della patch potrebbe essere
brillante e corretta, ma il suo veicolo testuale
(il diff) ne impedisce la materializzazione.

\paragraph{Il fallimento dei mutanti e l'overfitting}
L'ultimo approccio tradizionale è quello basato sui mutanti
(impiegato in framework come GenProg o SPR). Questi
sistemi alterano l'AST manipolando schemi predefiniti (operatori di
mutazione), ad esempio forzando una condizione a \texttt{true},
sostituendo un operatore relazionale o inserendo controlli di
nullità (\textit{null-checks}).

Questa strategia soffre di un doppio limite. In
primo luogo, lo spazio esplorativo è confinato al
ristretto set degli operatori implementati: se la risoluzione
del bug richiede la stesura di un'istruzione logica
non codificata da un mutante predefinito, la riparazione
è matematicamente impossibile. In secondo luogo, e in
maniera ancora più grave, i mutanti inducono un
massiccio \textit{overfitting} (sovradattamento). Le mutazioni casuali tendono a
generare patch che bypassano il caso di test
fallito in modo parziale o accidentale, alterando o
degradando silenziosamente comportamenti del programma per input non
coperti dai test. Ricerche empiriche dimostrano che il
tasso di \textit{overfitting} per alcuni di questi sistemi
può superare il 90\% delle patch giudicate plausibili.

\subsubsection{Introduzione a Tree-sitter: il Parser Incrementale ad Alta Fedeltà}
\label{subsubsec:3.4.5.2}
Per superare le fragilità intrinseche degli approcci testuali,
...
di \texttt{HybridRepair} richiede una gestione oculata delle risorse.
...
\begin{lstlisting}[language=Python, caption={Inizializzazione lazy del parser Tree-sitter tramite pattern singleton.}, label={lst:ensure_tree_sitter}]
...
lo stile originari adottati dagli ingegneri umani, riversando
...
Tree (CST). Questa struttura non scarta alcunché, vantando
...
nell'albero. Il pregio ingegneristico definitivo del CST si
...
\begin{table}[htbp]
\centering
\caption{Nodi Tree-sitter principali per l'analisi e manipolazione del codice Java.}
...
\begin{lstlisting}[language=Python, caption={Ricerca DFS iterativa per localizzare un nodo metodo tramite identificatore.}, label={lst:find_method_by_name_intro}]
...
nodi del tipo \texttt{method\_declaration} e \texttt{constructor\_declaration} estratti dal
...
sostituzione del relativo span di testo (\textit{text span}). L'ingegnerizzazione
di ciascuno di questi passaggi è stata accuratamente calibrata
per garantire la correttezza sintattica del risultato per costruzione,
assicurando l'affidabilità della patch a prescindere dalla complessità o
dalla verbosità del codice sorgente analizzato.
...
avviene tramite la funzione \texttt{\_ensure\_tree\_sitter()}. Questa funzione non è
isolata, ma è condivisa in modo architetturalmente trasversale tra
i moduli \texttt{ast\_applier.py} (responsabile della riparazione) e \texttt{prolog\_grounding.py} (che
...
\begin{lstlisting}[language=Python, caption={Parsing del codice sorgente con gestione rigorosa della codifica in byte.}, label={lst:parse_java}]
...
byte tramite la codifica UTF-8 (\texttt{source\_code.encode('utf-8')}) prima di
...
\begin{lstlisting}[language=Python, caption={Sostituzione chirurgica del codice sorgente sfruttando gli offset di Tree-sitter.}, label={lst:replace_via_ast}]
...
L'eleganza formale e la robustezza di questa funzione risiedono
...
\begin{lstlisting}[language=Python, caption={Ricerca del nodo bersaglio con gestione dell'annidamento e disambiguazione degli overload.}, label={lst:find_method_advanced}]
...
In secondo luogo, la funzione implementa un sofisticato meccanismo
...
risolvendo in modo univoco ed elegante l'ambiguità degli overload
multipli.
...
non rappresenta un mero vezzo estetico, ma costituisce un
...
dedicate: \texttt{\_detect\_indentation()} per campionare lo stile originario e \texttt{\_reindent()}
per applicarlo dinamicamente al codice generato.

\begin{lstlisting}[language=Python, caption={Rilevamento automatico dell'indentazione del codice originale.}, label={lst:detect_indentation}]
...
\begin{lstlisting}[language=Python, caption={Riallineamento relativo del codice generato dall'LLM.}, label={lst:reindent}]
...
Un aspetto ingegneristico fondamentale è la differenza tra un'imposizione
...
\begin{lstlisting}[language=Python, caption={Calcolo del diff unificato diagnostico post-sostituzione.}, label={lst:compute_diff}]
...
metodo implementa una sofisticata strategia di fallback graduale a
due livelli (\textit{graceful degradation}):

\begin{lstlisting}[language=Python, caption={Orchestrazione della sostituzione con fallback degradante.}, label={lst:apply_method_replacement}]
...
approccio basato su espressioni regolari (che ricalca i meccanismi
della pipeline v1 di \texttt{HybridRepair}). Il fallback regex opera
...
\begin{lstlisting}[language=Python, caption={Estrazione e risoluzione del classpath con supporto alla glob expansion.}, label={lst:read_classpath}]
...
La ratio ingegneristica di questo blocco di codice risiede
nella sua resilienza di fronte all'eterogeneità dei progetti. Il
file \texttt{config.properties} definisce il classpath utilizzando percorsi relativi alla
directory del bug, concatenandoli tipicamente in una forma del
tipo \texttt{class.path=../../lib/junit-4.11.jar:../../lib/hamcrest-core-1.3.jar:...}. La funzione si occupa di ancorare
...
\begin{lstlisting}[language=Python, caption={Isolamento dei file alterati e compilazione selettiva tramite \texttt{javac}.}, label={lst:compile_patched_source}]
...
\begin{lstlisting}[language=Python, caption={Esecuzione dei test automatizzati in ambiente sandbox con limite temporale.}, label={lst:run_junit}]
...
di scudo ingegneristico contro un fenomeno endemico dell'Automated Program
...
\begin{lstlisting}[language=Python, caption={Parsing con espressioni regolari del log JUnit per l'estrazione delle metriche semantiche.}, label={lst:parse_junit_output}]
...
L'algoritmo impiega un'espressione regolare chirurgica (\verb|\.E\s+...|) per ispezionare
...
5) offre un indicatore di progresso ad altissima granularità.
Esso segnala all'agente (e al ricercatore durante la fase
di debugging post-hoc) che la rotta esplorativa è parzialmente
...
da veri e propri archivi permanenti e onnicomprensivi. Di
seguito viene presentata la definizione architetturale di queste due
entità:

\begin{lstlisting}[language=Python, caption={Definizione formale delle strutture dati per la memorizzazione degli esiti.}, label={lst:models_patch_repair}]
...
La struttura \texttt{PatchAttempt} è stata meticolosamente progettata per abilitare
...
modulo \texttt{services/reporter.py}, il quale serializza ricorsivamente l'oggetto \texttt{RepairResult} in
...
\begin{description}
    ...
    in una directory sandbox vergine (\texttt{pipeline\_results/<bug\_id>/attempt\_k/patched\_source/}), azzerando il rischio
    ...
    strumenti (\texttt{create\_tool\_executor()}) e si forgia il prompt iniziale tramite
    \texttt{\_build\_messages()}. Crucialmente, se l'agente si trova in un tentativo
    ...
    9 iterazioni (\texttt{max\_tool\_rounds}). Durante ogni round, il modello transita
    fluidamente tra gli stati della FSM (\texttt{EXPLORE}, \texttt{PATCH}, \texttt{COMPILE},
    ...
    \item[Persistenza ed Esame:] Conclusa l'interazione generativa, \texttt{conversation\_store.save()} interviene per
    serializzare l'intero scambio in un robusto log JSONL. Immediatamente
...
\label{subsec:3.4.10}
L'architettura del \texttt{PatchAgent}, per quanto sofisticata, non è esente
da compromessi ingegneristici. Un'analisi rigorosa delle prestazioni del sistema
impone di valutarne criticamente sia i trionfi architetturali, sia
le limitazioni strutturali e teoriche emerse durante le validazioni
empiriche.

\subsubsection{Superiorità Strutturale dell'Approccio Tree-sitter}
\label{subsubsec:3.4.10.1}
Il traguardo ingegneristico più significativo della Fase 3 risiede,
senza dubbio, nell'adozione di Tree-sitter per la manipolazione strutturale
del codice sorgente. I vantaggi di questo approccio rispetto
alle metodologie testuali classiche si sono dimostrati sia teoricamente
solidi che empiricamente inoppugnabili. Dal punto di vista teorico,
l'operazione di sostituzione fondata sui \textit{byte-offset} del \textit{Concrete Syntax
Tree} (CST) fornisce una garanzia matematica di correttezza sintattica
per costruzione. Sul piano empirico, la transizione dalla pipeline
di prima generazione (v1, basata su espressioni regolari e
contatori euristici di parentesi) all'attuale modulo \texttt{ast\_applier.py} ha eradicato
in modo definitivo il cosiddetto problema P1. Nella versione
precedente del sistema, circa il 30\% dei tentativi di
riparazione veniva invalidato da errori sintattici grossolani, quali parentesi
graffe sbilanciate o dichiarazioni di metodi frammentate e duplicate.
Nella pipeline v2, questo fenomeno di corruzione sintattica è
totalmente scomparso. A ciò si aggiunge il beneficio tangibile
derivante dalla preservazione dell'indentazione originale tramite le routine \texttt{\_detect\_indentation()}
e \texttt{\_reindent()}, che producono diff estremamente compatti e leggibili,
abbattendo il rischio di regressioni stilistiche in ecosistemi governati
da rigide policy di \textit{Continuous Integration}.

\subsubsection{Limitazioni del Fallback Regex}
\label{subsubsec:3.4.10.2}
Nonostante la robustezza del livello primario, la strategia di
fallback basata su espressioni regolari (\texttt{\_find\_method\_bounds\_regex()}) eredita tutte le
vulnerabilità intrinseche degli approcci puramente testuali. Questo meccanismo secondario
garantisce un comportamento corretto quasi esclusivamente in presenza di
metodi caratterizzati da un singolo livello di annidamento logico.
Di fronte a strutture sintattiche complesse, tipiche del codice
\textit{enterprise}, come classi anonime o interfacce annidate all'interno di
metodi omonimi, l'algoritmo regex rischia di calcolare confini posizionali
errati. Questa imprecisione teorica può degenerare nell'inclusione o esclusione
accidentale di porzioni di codice vitali. La conservazione di
questo modulo nell'architettura è giustificata unicamente da ragioni di
resilienza estrema: in ambienti operativi vincolati dove l'installazione delle
dipendenze binarie native in C (necessarie per \texttt{tree-sitter-java}) risulta
preclusa, il sistema deve poter tentare una riparazione in
regime di \textit{graceful degradation}, pur accettando una drastica riduzione
delle garanzie formali.

\subsubsection{Il Problema del Budget di Tool-Call e della Profondità Esplorativa}
\label{subsubsec:3.4.10.3}
Un'ulteriore limitazione prestazionale è legata alla configurazione della Macchina
a Stati Finiti, specificamente al budget massimo di interazioni
concesso all'agente. L'assegnazione di 9 round di \textit{tool-call} per
ciascun tentativo (\texttt{MAX\_AGENT\_STEPS * 3}) rappresenta un compromesso pragmatico
tra la necessità di una profonda esplorazione contestuale e
l'imperativo di contenere i costi computazionali e finanziari delle
API. Tuttavia, l'analisi empirica ha evidenziato come questo budget
risulti insufficiente per la risoluzione di difetti che esigono
un'indagine \textit{cross-class} particolarmente estesa. Bug notoriamente complessi all'interno del
dataset Defects4J, quali \texttt{Jsoup-22}, \texttt{JacksonDatabind-80} o \texttt{Lang-33}, traggono origine
da radici causali (come valori nulli o stati inconsistenti)
che si propagano attraverso gerarchie di ereditarietà multi-livello, spesso
originate in superclassi astratte non fornite nel contesto iniziale
del prompt. In queste istanze, l'agente esaurisce sistematicamente i
propri round esplorativi senza riuscire a sintetizzare la patch,
marcando il tentativo come fallito per timeout operazionale. Sebbene
un innalzamento del parametro \texttt{MAX\_AGENT\_STEPS} estenderebbe l'orizzonte investigativo dell'agente,
ciò comporterebbe un incremento lineare e potenzialmente insostenibile dei
costi di inferenza per tentativo.

\subsubsection{Dipendenza dall'Accuratezza della \texttt{RepairSpec}}
\label{subsubsec:3.4.10.4}
Infine, l'architettura di \texttt{PatchAgent} soggiace al principio assiomatico dell'ingegneria
del software noto come ``\textit{garbage-in, garbage-out}''. L'agente elegge la
\texttt{RepairSpec} (ricevuta dalla Fase 2) a propria direttiva concettuale
suprema, declinandola in codice sorgente senza sottoporla a un'ulteriore
validazione formale. Qualora le fasi diagnostiche antecedenti producessero una
specifica fallace, derivata da catene causali incomplete formulate da
\texttt{LogicFL} o da un travisamento semantico perpetrato da \texttt{SpecBuilder},
il \texttt{PatchAgent} orienterebbe inesorabilmente i propri sforzi verso una
soluzione non corretta. Sebbene l'iniezione della memoria conversazionale e
l'uso del \textit{negative prompting} (\texttt{conversation\_store.py}) stimolino il modello a
divergere da una strategia iniziale inefficace nei tentativi successivi,
esse non assicurano la convergenza verso il fix corretto
se l'assunto di partenza è fondamentalmente errato. L'integrazione di
moduli di auditing preliminare, come \texttt{SpecCritic}, attenua sensibilmente questa
vulnerabilità sistemica, ma non è in grado di neutralizzarla
del tutto.
...
\begin{table}[htbp]
\centering
\caption{Sintesi dei contributi tecnici della Fase 3 di \texttt{HybridRepair}.}
...
La superiorità architetturale derivante dall'utilizzo di Tree-sitter rispetto alle
euristiche testuali classiche eleva la manipolazione del codice a
un livello di rigore matematico. Sostituendo chirurgicamente \textit{text span}
delimitati da \textit{byte-offset} all'interno del CST, il sistema sancisce
due proprietà formali inalienabili: la correttezza sintattica per costruzione
dell'artefatto risultante e la perfetta preservazione dello stile e
dell'indentazione del progetto ospite.

L'orchestrazione di queste componenti culmina in una Macchina a
Stati Finiti implicita che guida un LLM attraverso gli
stati di \texttt{EXPLORE}, \texttt{PATCH}, \texttt{COMPILE}, \texttt{TEST} e \texttt{RETRY}. All'interno
di questo ciclo vitale, l'agente trascende la mera generazione
stocastica per divenire un'entità reattiva, capace di auto-correggere le
proprie sviste interpretando autonomamente i verdetti deterministici emessi dal
compilatore e dal test runner. La sintesi tra l'adattamento
termico (\textit{schedule} di temperatura), l'isolamento ambientale (sandbox) e il
meta-apprendimento indotto dalla memoria conversazionale JSONL dota \texttt{HybridRepair} di
una reale capacità di apprendimento intra-sessione. È proprio questa
attitudine auto-correttiva e riflessiva a elevare \texttt{HybridRepair} oltre i
confini dei sistemi APR di prima generazione, consolidando un
paradigma in cui l'intelligenza artificiale generativa è finalmente imbrigliata
dai vincoli rigorosi dell'ingegneria del software.